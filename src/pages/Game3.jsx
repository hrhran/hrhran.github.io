...
export default function Game3() {
      ...
      const i = setInterval(() => {
        setBootProgress((p) => {
          ...
          if (p >= 100) {
            clearInterval(i)
            setTimeout(() => {
              const a = parseInt(localStorage.getItem('game3_attempts') || '0')
              localStorage.setItem('game3_attempts', (a + 1).toString())
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      if (p.y > H + 100) { p.x = 100; p.y = 400; p.vx = 0; p.vy = 0 }
      ...
      <div className="w-screen h-screen bg-black flex items-center justify-center">
        <div className="w-full max-w-2xl px-8">
          ...
          <div className="text-center mb-12">
            <div className="text-white text-4xl mb-2 font-bold" style={{ fontFamily: 'Arial, sans-serif' }}>
              ...
              <span className="text-white mx-2">95</span>
            </div>
            <div className="text-gray-500 text-sm font-mono">CRS Corporate Edition</div>
          </div>

          {/* Boot Messages */}
          <div className="bg-gray-900 border-2 border-gray-700 p-6 mb-6 font-mono text-sm">
            <div className="text-green-400 mb-2">
              <span className="animate-pulse">▶</span> {bootMessage}
            </div>
            <div className="text-gray-600 text-xs space-y-1 mt-4">
              {bootProgress > 5 && <div>✓ System BIOS loaded</div>}
              {bootProgress > 15 && <div>✓ Network adapter initialized</div>}
              {bootProgress > 25 && <div>✓ Employee ID verified: EMP-{Math.floor(Math.random() * 9000 + 1000)}</div>}
              {bootProgress > 35 && <div>✓ Database connection established</div>}
              {bootProgress > 45 && <div>✓ Security protocols active</div>}
              {bootProgress > 55 && <div>✓ HR system online</div>}
              {bootProgress > 65 && <div>✓ Headquarters sync complete</div>}
              {bootProgress > 75 && <div>✓ Email client ready</div>}
              {bootProgress > 85 && <div>✓ User profile loaded</div>}
            </div>
          </div>

          {/* Progress Bar */}
          <div className="mb-4">
            <div className="w-full h-10 bg-gray-800 border-2 border-gray-600 relative overflow-hidden">
              <div
          ...
          {/* System Info */}
          <div className="text-gray-600 text-xs font-mono text-center space-y-1">
            <div>CRS Employee Portal v2.1.4</div>
            <div>Copyright © 1995 Consumer Recreational Services</div>
            <div className="text-gray-700 mt-2">Please wait while the system prepares your workspace...</div>
          </div>
        </div>
      </div>
    )
